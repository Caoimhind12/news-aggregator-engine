...
bbc_r1 = rq.get('https://www.bbc.co.uk/news/politics')
coverpage_bbc = bbc_r1.content
soup_bbc = BeautifulSoup(coverpage_bbc,'html5lib')
bbc_cover_articles = soup_bbc.find_all('div',class_='gs-c-promo gs-t-News nw-c-promo gs-o-faux-block-link gs-u-pb gs-u-pb+@m nw-p-default gs-c-promo--inline gs-c-promo--stacked@m gs-c-promo--flex')
...
for n in np.arange(0,number_of_articles):
    #getting the links
    link_bbc_politics = bbc_cover_articles[n].find('a')['href']
    links_bbc.append(link_bbc_politics)

    #getting the tiles
    title = bbc_cover_articles[n].find('a').get_text()
    title_bbc.append(title)

#go to inside the detail view of news
    article = rq.get("https://www.bbc.co.uk"+link_bbc_politics)
    ...
    add_news = ("INSERT IGNORE INTO news"
                ...
                "VALUES (%s, %s, %s, %s, %s , %s)")
    data_news = ("https://www.bbc.co.uk"+link_bbc_politics,"1", 'bbc',title, final_article, date_publish_baseline[0].get_text())
    #insertion
